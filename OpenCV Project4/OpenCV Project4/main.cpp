...
void drawText1() {
	...
	putText(img, "FONT_HERSHEY_COMPLEX | FONT_ITALIC", Point(20, 450), FONT_HERSHEY_COMPLEX | FONT_ITALIC, 1, Scalar(255, 0, 255));
...
void keyEvent1() {
	...
	cout << "[i] or [I] 버튼을 누르면 이미지가 반전됩니다." << endl;
	cout << "종료하려면 [Esc] or [q] or [Q] 버튼을 눌러주세요." << endl;

	while (true) {
		//특수 키( f1 ~ f12 또는 화살표 키 등) = waitKeyEx() 함수 사용
		int keycode = waitKey();
...
void on_mouse(int event, int x, int y, int flags, void*) {
	...
	case EVENT_MOUSEMOVE:
		if (flags & EVENT_FLAG_LBUTTON) {
			line(imgMouse, pt0ld, Point(x, y), Scalar(0, 255, 255), 2);
			imshow("img", imgMouse);
			pt0ld = Point(x, y);
		}
		break;
